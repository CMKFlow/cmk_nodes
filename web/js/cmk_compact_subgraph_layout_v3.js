...
function install(node) {
    if (!isCompactFlowModule(node) || node.__cmkCompactLayoutV3) return;
    node.__cmkCompactLayoutV3 = true;
    ensureAdvancedSpacer(node);
    const originalArrangeWidgets = node._arrangeWidgets;
    node._arrangeWidgets = function() {
        const result = originalArrangeWidgets?.apply(this, arguments);
        applyLayout(this);
        return result;
    };
    node.arrange?.();
...
app.registerExtension({
    name: "cmk.compact_subgraph_layout.v3",
    setup() {
        setInterval(() => {
