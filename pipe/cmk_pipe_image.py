...
class CMKPipeCreateImage:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                "GLOBAL PROMPT POS": ("STRING", {"default": "", "multiline": True, "tooltip": "Positive prompt for the complete Flow."}),
                ...
                "instantid_enabled": (
                    "BOOLEAN",
                    {
                        ...
                        "label": "INSTANTID",
                        "tooltip": (
                            "SDXL Text2Image or Inpaint. Activates module 15 and makes module 10 "
                            "use the InstantID layout preset."
                        ),
                    },
                ),
                "instantid_mode": (
                    ...
                    {
                        "default": "HYBRID",
                        "label": "INSTANTID MODE",
                        "tooltip": (
                            "Text2Image only. HYBRID lets module 10 establish the composition; "
                            "ZERO-PASS lets module 15 create it directly. Inpaint always uses "
                            "the dedicated HYBRID INPAINT bridge."
                        ),
                    },
                ),
                # Appended so saved Create Image widget values stay positional.
                "inpaint_mask_expand": (
                    "INT",
                    {
                        "default": 16,
                        ...
                        "step": 1,
                        "advanced": True,
                        "label": "INPAINT MASK EXPAND",
                        "tooltip": (
                            "Expands only the sampler's Inpaint region beyond the drawn "
                            "mask edge to prevent VAE/latent boundary seams."
                        ),
                    },
                ),
            },
    ...
    def create_image(self, **inputs):
        ...
        instantid_enabled = bool(inputs.get("instantid_enabled", False))
        inpaint_mask_expand = max(
            4, min(32, int(inputs.get("inpaint_mask_expand", 16) or 16))
        )
        requested_instantid_mode = str(inputs.get("instantid_mode", "HYBRID") or "HYBRID").strip().upper()
        instantid_mode = (
            "HYBRID INPAINT"
            if INPAINT_MODE
            else ("ZERO-PASS" if requested_instantid_mode == "ZERO-PASS" else "HYBRID")
        )
        ...
        pipe.update({
            ...
            "instantid_enabled": bool(
                instantid_enabled and model_family == "sdxl"
            ),
            "instantid_mode": instantid_mode,
            "inpaint_mask_expand": inpaint_mask_expand,
        })
