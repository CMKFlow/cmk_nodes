...
def _estimate_norm_without_deprecated_api(lmk, image_size=112, mode='arcface'):
    """InsightFace's estimate_norm using the current scikit-image API.

    insightface 1.0.1 still calls ``SimilarityTransform.estimate``, which is
    deprecated in scikit-image 0.26.  Keep the calculation local and otherwise
    identical to insightface.utils.face_align.estimate_norm.
    """
    ...
    if image_size % 112 == 0:
        ...
        diff_x = 8.0 * ratio
    dst = insightface_face_align.arcface_dst * ratio
    dst[:, 0] += diff_x
    transform_type = skimage_transform.SimilarityTransform
    if hasattr(transform_type, "from_estimate"):
        tform = transform_type.from_estimate(lmk, dst)
    else:
        # Compatibility with scikit-image releases predating from_estimate.
        tform = transform_type()
        tform.estimate(lmk, dst)
    return tform.params[0:2, :]


# arcface_onnx calls face_align.norm_crop, whose global estimate_norm lookup is
# resolved in this module.  Replace only that deprecated helper; no installed
# package file is modified.
insightface_face_align.estimate_norm = _estimate_norm_without_deprecated_api


def _keypoint_images_to_tensor(images):
    # functional.to_tensor is the exact implementation used by the deprecated
    # ToTensor transform.  The suggested v2 ToDtype replacement differs by up
    # to one float32 ULP, so retain the established InstantID conditioning.
    return torch.stack(
        [torchvision_functional.to_tensor(image) for image in images], dim=0
    ).permute([0, 2, 3, 1])
...
def _select_detected_face(faces, selection="Largest"):
    selection = str(selection or "Largest")
    if selection == "Leftmost":
        return min(faces, key=lambda face: (face["bbox"][0] + face["bbox"][2]) / 2)
    if selection == "Rightmost":
        return max(faces, key=lambda face: (face["bbox"][0] + face["bbox"][2]) / 2)
    if selection == "Topmost":
        return min(faces, key=lambda face: (face["bbox"][1] + face["bbox"][3]) / 2)
    if selection == "Bottommost":
        return max(faces, key=lambda face: (face["bbox"][1] + face["bbox"][3]) / 2)
    if selection == "Center":
        centers = [((face["bbox"][0] + face["bbox"][2]) / 2, (face["bbox"][1] + face["bbox"][3]) / 2) for face in faces]
...
def _regional_face_mask(height, width, bbox):
    ...
    blur = max(8, int(min(max(1, x2 - x1), max(1, y2 - y1)) * 0.18))
...
def extractFeatures(insightface, image, extract_kps=False, selection="Largest", return_mask=False):
    ...
    for i in range(face_img.shape[0]):
        for size in [(size, size) for size in range(640, 128, -64)]:
            ...
            if face:
                face = _select_detected_face(face, selection)

                if extract_kps:
                    out.append(draw_kps(face_img[i], face['kps']))
                    if return_mask:
                        return (
                            _keypoint_images_to_tensor(out),
                            _regional_face_mask(face_img[i].shape[0], face_img[i].shape[1], face['bbox']),
                        )
                else:
    ...
    if out:
        if extract_kps:
            out = _keypoint_images_to_tensor(out)
        else:
...
class ApplyInstantID:
    ...
    def apply_instantid(self, instantid, insightface, control_net, image, model, positive, negative, start_at, end_at, weight=.8, ip_weight=None, cn_strength=None, noise=0.35, image_kps=None, mask=None, combine_embeds='average', target_selection="Largest", image_kps_is_hint=False):
        ...
        keypoint_result = image_kps if image_kps_is_hint else extractFeatures(
            insightface,
            image_kps if image_kps is not None else image[0].unsqueeze(0),
            extract_kps=True,
            selection=target_selection,
            return_mask=mask is None,
        )
        if isinstance(keypoint_result, tuple):
            face_kps, mask = keypoint_result
        else:
            face_kps = keypoint_result
        ...
        for conditioning in [positive, negative]:
            ...
            for t in conditioning:
                ...
                # Keep ordinary positive prompt guidance global. The regional
                # mask is consumed by the InstantID attention patch above;
                # attaching it here would leave a rectangular noisy frame.
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "target_selection": (("Largest", "Leftmost", "Rightmost", "Topmost", "Bottommost", "Center"), {"default": "Largest"}),
            },
            "optional": {
                "image_kps": ("IMAGE",),
                "mask": ("MASK",),
